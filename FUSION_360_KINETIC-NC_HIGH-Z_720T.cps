...
description = "Post processor for the KinetiC-NC software (modified by chiefenne)";
vendor = "chiefenne";
...
if (!longDescription) {
    longDescription = subst("Generic %1. The main feature is a safe path to the workpiece origin.", description);
}
...
function onOpen() {
  ...
  writeln("This G-code is the result of a post-processor modification by chiefenne.");
    writeln("The original post-processor from CNC-STEP (version 45917) was modified to include additional features.");
...
function onSection() {
  ...
  if (insertToolCall) {
      forceModals();
      // >>> chiefenne
      // to force M66 for manual tool change
  ...
  if (getProperty("writeTools")) {
    if (false) { // set to true to use the post kernel version of the tool list
      ...
      if (tools.getNumberOfTools() > 0) {
        ...
        // >>> chiefenne
        writeln("");
        writeComment("Safe path to workpiece origin - typically G54");
        writeln("G54");
...
// <<<<< INCLUDED FROM include_files/workPlaneFunctions_fanuc.cpi
// >>>>> INCLUDED FROM include_files/N1145 G28 G91 Z0
...
// >>> chiefenne
function safeStartPositionChiefenne(options = {}) {
    ...
    //
    // call possibilites:
    //   safeStartPositionChiefenne() ... move to safe position in X,Y,Z
    //   safeStartPositionChiefenne({useG28: true}) ... move to safe position in X,Y,Z and use G28 (move to machine zero
    //
    //
    const {useG28 = false } = options;
    ...
    if (useG28) {
        writeBlock(gFormat.format(28)); // move to machine zero
    }
}
// <<< chiefenne
