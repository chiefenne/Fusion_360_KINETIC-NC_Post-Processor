...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(gFormat.format(53), gMotionModal.format(0), "Z" + xyzFormat.format(0)); // retract
    zOutput.reset();
  }
  ...
  writeln("(Skip label must exist, then jump label is accepted)");
  writeln("(SKIP Q0000)");
  writeln("");
